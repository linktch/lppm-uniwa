<div class="min-h-screen bg-gray-50 p-6" x-data="autoSave()" x-init="initAutoSave()">
    <div class="max-w-4xl mx-auto">
        ...
        <!-- Alert Auto-Save -->
        <div x-show="showAutoSaveAlert" x-cloak
            class="fixed bottom-4 right-4 z-50 bg-green-500 text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-2"
            style="display: none;">
            <i class="fas fa-save"></i>
            <span class="text-sm">Data otomatis tersimpan</span>
        </div>

        <div x-show="showRestoreAlert" x-cloak class="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-xl mb-6"
            style="display: none;">
            <div class="flex items-start gap-3">
                <i class="fas fa-history text-yellow-500 text-lg"></i>
                <div class="flex-1">
                    <p class="text-sm text-yellow-700 font-medium">Ada data draft yang belum selesai!</p>
                    <p class="text-xs text-yellow-600 mt-1">Apakah Anda ingin melanjutkan mengisi data sebelumnya?</p>
                    <div class="flex gap-2 mt-3">
                        <button @click="restoreAutoSave()"
                            class="px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-xs">
                            <i class="fas fa-undo"></i> Ya, Lanjutkan
                        </button>
                        <button @click="clearAutoSave()"
                            class="px-3 py-1 bg-gray-300 hover:bg-gray-400 text-gray-700 rounded-lg text-xs">
                            <i class="fas fa-trash"></i> Tidak, Mulai Baru
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden mb-6">
            <div class="px-5 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-lg bg-gradient-to-r from-blue-700 to-blue-500 flex items-center justify-center">
            ...
            <div class="p-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <div class="relative">
                            <i
                                class="fas fa-calendar-day absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
                            <input type="date" wire:model.live="tanggal" x-on:change="saveToLocalStorage()"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="time" wire:model.live="jam" x-on:change="saveToLocalStorage()"
            ...
            <div class="p-5">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <textarea wire:model.live.debounce.500ms="narasi" rows="6" x-on:input="saveToLocalStorage()"
                    ...
                    <div class="flex justify-between items-center mt-1">
                        ...
                        <div class="text-xs text-gray-500">
                            <i class="fas fa-keyboard"></i> <span
                                x-text="$wire.narasi ? $wire.narasi.length : 0"></span> / Minimal 500 karakter
                        </div>
                    </div>
                    @error('narasi')
                    <div class="flex items-center gap-1 mt-1 text-xs text-red-500">
...
<div class="bg-white rounded-xl shadow-md overflow-hidden mb-6">
    ...
    <div class="p-5">
        <div>
            ...
            <!-- NOTIFIKASI: Foto harus dengan GPS -->
<div class="mb-4 bg-blue-50 border-l-4 border-blue-500 p-3 rounded-lg">
    <div class="flex items-start gap-2">
        ...
        <div class="text-xs text-blue-700">
            <p class="font-semibold mb-1">📸 Ketentuan Foto:</p>
            <p>Foto harus menyertakan <strong>lokasi, tanggal, dan waktu</strong> (gunakan kamera HP atau Google Foto)</p>
        </div>
    </div>
</div>

            <!-- Area Upload - HIDDEN jika sudah ada foto -->
            <div x-data="{ isDragging: false }" 
                 x-show="!$wire.foto && !$wire.oldFoto"
                ...
                <input type="file" wire:model="foto" accept="image/*" style="display: none;" x-ref="fotoInput" @change="handleFileSelect($event)">
                ...
                <div wire:loading.remove wire:target="foto">
                    ...
                    <small class="text-gray-400">Format: JPG, PNG, JPEG (Max. 1MB)</small>
                </div>
            ...
            <!-- Preview Foto (hanya 1 foto) -->
            @if ($foto && !is_string($foto))
                <div class="mt-4 relative inline-block">
                    <div class="relative">
                        <img src="{{ $foto->temporaryUrl() }}" alt="Preview Foto" 
                             class="w-48 h-36 object-cover rounded-xl border-2 border-green-400 shadow-sm">
            ...
            @elseif($oldFoto)
                <div class="mt-4 relative inline-block">
                    <div class="relative">
                        <img src="{{ Storage::url($oldFoto) }}" alt="Foto Kegiatan" 
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="button" wire:click="saveLaporan" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove>
                            <i class="fas fa-save mr-1"></i> {{ $isEditing ? 'Update Laporan' : 'Kirim Laporan' }}
                        </span>
                        <span wire:loading>
...
function autoSave() {
    return {
        ...
        initAutoSave() {
            ...
            this.$watch('$wire.tanggal', () => this.saveToLocalStorage());
            this.$watch('$wire.jam', () => this.saveToLocalStorage());
            this.$watch('$wire.narasi', () => this.saveToLocalStorage());
        },

        hasData() {
            const tanggal = this.$wire.get('tanggal');
            const jam = this.$wire.get('jam');
            const narasi = this.$wire.get('narasi');
            return tanggal || jam || narasi;
        },

        saveToLocalStorage() {
            try {
                const draft = {
                    tanggal: this.$wire.get('tanggal'),
                    jam: this.$wire.get('jam'),
                    narasi: this.$wire.get('narasi'),
                    saved_at: new Date().toISOString()
                };
                localStorage.setItem('laporan_harian_draft', JSON.stringify(draft));

                this.showAutoSaveAlert = true;
                setTimeout(() => {
                    this.showAutoSaveAlert = false;
                }, 2000);
            } catch (e) {
                console.warn('Auto-save gagal:', e);
                if (e.name === 'QuotaExceededError') {
                    localStorage.removeItem('laporan_harian_draft');
                }
            }
        },

        checkForDraft() {
            const isEditing = this.$wire.get('isEditing');
            const draft = localStorage.getItem('laporan_harian_draft');
            if (draft && !isEditing) {
                try {
                    const data = JSON.parse(draft);
                    if (data.tanggal || data.jam || data.narasi) {
                        this.showRestoreAlert = true;
                    }
                } catch (e) {
                    console.warn('Gagal parse draft:', e);
                }
            }
        },
        ...
        restoreAutoSave() {
            ...
            if (draft) {
                try {
                    const data = JSON.parse(draft);
                    this.$wire.set('tanggal', data.tanggal || '');
                    this.$wire.set('jam', data.jam || '');
                    this.$wire.set('narasi', data.narasi || '');
        ...
        handleFileSelect(event) {
            ...
            if (file) {
                if (file.size > 1 * 1024 * 1024) { // 1MB = 1048576 bytes
