    ...
    <div class="px-4 mt-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <div class="flex items-center gap-2">
                <span
                    class="inline-flex items-center gap-2 px-3 py-2 bg-white text-gray-700 text-sm rounded-full shadow-sm">
                ...
                <div class="p-4">
                    ...
                    <div class="mt-2">
                        <div class="flex justify-between text-xs mb-1">
                            <span>Progress</span>
                            <span
                                class="font-semibold {{ $progress < 30 ? 'text-red-600' : ($progress < 70 ? 'text-yellow-600' : 'text-green-600') }}">
                                {{ $progress }}%
                            </span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="h-2 rounded-full"
                                style="width: {{ $progress }}%; background: linear-gradient(90deg, #ef4444, #f59e0b, #10b981);">
                            </div>
                        </div>
                        <div class="flex justify-between text-xs text-gray-500 mt-1">
                            <span><i class="fas fa-check-circle text-green-500"></i>
                                {{ $sudahDinilai }}/{{ $totalIndikator }}</span>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="inline-flex px-2 py-1 text-xs rounded-full {{ $mahasiswa->jenis_kelamin == 'L' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }}">
                                    <i
                                        class="fas {{ $mahasiswa->jenis_kelamin == 'L' ? 'fa-mars' : 'fa-venus' }} mr-1"></i>
                            ...
                            <td class="px-4 py-3">
                                <div class="w-40">
                                    <div class="flex justify-between text-xs mb-0.5">
                                        ...
                                        <span
                                            class="font-semibold {{ $progress < 30 ? 'text-red-600' : ($progress < 70 ? 'text-yellow-600' : 'text-green-600') }}">{{ $progress }}%</span>
                                    </div>
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div class="h-2 rounded-full"
                                            style="width: {{ $progress }}%; background: linear-gradient(90deg, #ef4444, #f59e0b, #10b981);">
                                    ...
                                    <div class="flex justify-between text-xs text-gray-500 mt-1">
                                        <span><i class="fas fa-check-circle text-green-500"></i>
                                            {{ $sudahDinilai }}/{{ $totalIndikator }}</span>
                                        <span><i class="fas fa-star text-yellow-500"></i>
                                            {{ $totalNilai }}/{{ $maxNilai }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button
                                        wire:click="openPasswordModal({{ $mahasiswa->id }}, '{{ $mahasiswa->nama }}')"
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <!-- Field password di modal -->
                        <input type="password" wire:model="password" wire:keydown.enter="generateSertifikat"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                            placeholder="Masukkan password digital signature" autocomplete="new-password"
                            name="sertifikat_password" id="sertifikat_password">
                    ...
                    <div class="bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
                        <div class="flex items-start gap-2">
                            ...
                            <p class="text-xs text-yellow-700">Password digunakan untuk otorisasi penerbitan sertifikat
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 p-4 border-t bg-gray-50">
                    <button type="button" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-lg text-sm"
                        wire:click="closePasswordModal">Batal</button>
                    <button type="button"
                        class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm"
                        wire:click="generateSertifikat" wire:loading.attr="disabled">
