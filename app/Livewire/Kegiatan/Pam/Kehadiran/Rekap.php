...
class Rekap extends Component
{
    ...
    public function render()
    {
        ...
        // dd($kelompokUsers);
        // KEHADIRAN
        $kehadiran = Kehadiran::whereIn('user_id', $userIds)
        ...
        $dataMahasiswa->setCollection(
            $dataMahasiswa->getCollection()->map(function ($user) use ($kelompokUsers, $kehadiran) {
                ...
                foreach ($absen as $item) {
                    ...
                    match ($hari) {
                        'monday' => $hadir['senin'] = (bool) $item->status,
                        'tuesday' => $hadir['selasa'] = (bool) $item->status,
                        'wednesday' => $hadir['rabu'] = (bool) $item->status,
                        'thursday' => $hadir['kamis'] = (bool) $item->status,
                        default => null,
                    };
                }
                ...
                return [
                    ...
                    'hadir' => $hadir,
                    'total_hadir' => collect($hadir)->filter()->count(),
                ];
            })
        );
        // dd($kehadiran);
        ...
        return view('livewire.kegiatan.pam.kehadiran.rekap', compact(
            'kelompoks',
            'periodes',
            'kegiatan',
            'prodis',
